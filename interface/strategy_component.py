# This is my code that need debugging.
...
#                 self._all_contracts.append(symbol + "-" + exchange.capitalize())

#         self._all_contracts = ["BTCUSDT", "ETHUSDT"]
#         self._all_timeframes = ["1m", "5m", "15m", "30m", "1h", "4h"]

#         self._commands_frame = tk.Frame(self, bg=BG_COLOR)
#         self._table.pack(side=tk.TOP)

#         self._table_frame = tk.Frame(self, bg=BG_COLOR)
#         self._table_frame.pack(side=tk.TOP)

#         self._add_button = tk.Button(self._commands_frame, text="Add strategy", font=GLOBAL_FONT, command=self._add_strategy_row, bg=BG_COLOR_2, fg=FG_COLOR)
...
#             {"code_name": "parameters", "widget": tk.Entry, "data_type": float, "text": "Parameters", 
#              "bg"= BG_COLOR_2, "command": self._show_popup},
#             {"code_name": "activation", "widget": tk.Entry, "data_type": float, "text": "OFF", 
#              "bg"= "darkred", "command": self._switch_strategy},
#             {"code_name": "delete", "widget": tk.Entry, "data_type": float, "text": "X", 
...
#                 {"code_name": "min_volume", "name": "Minimum value", "widget": tk.Entry, "data_type": float}
#             ],
#         }

#         for idx, h in enumerate(self._headers):
#             header = tk.Label(self._table_frame, txt=h, bg=FG_COLOR, fg=FG_COLOR, font=BOLD_FONT)
...
#                 self.body_widgets[code_name][b_index] = tk.OptionMenu(self._table_frame, self.body_widgets[code_name + "var"][b_index], *base_param['values'])
...
#         self._popup_window.config("-topmost", "true")
...
#                 self._extra_input[code_name] = tk.Entry(self._popup_window, bg=BG_COLOR, justify=tk.CENTER, fg=FG_COLOR, insertbackgroup=FG_COLOR)

#                 if self._additional-parameters[b_index][code_name] is not None:
#                     self._extra_input[code_name].insert(tk.END, str(self._additional-parameters[b_index][code_name))

#         else:
#             continue

#         self._extra_input[code_name].grid(row=row_nd, column=1)
...
#         symbol = self.body_widgets['contract_var'][b_index].get().split("-")[0]
#         timeframe = self.body_widgets['timeframe_var'][b_index].get()
#         exchange = self.body_widgets['contract_var'][b_index].get().split("-")[1]
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets['activation'][b_index].cget("text") == "OFF":
            ...
            if len(new_strategy.candles) == 0:
                   self.root.logging_frame.add_log(f"No historical data available for {contract.symbol}")
                   return
