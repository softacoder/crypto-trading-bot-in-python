    ...
    def _changed(self, var_name: str, index: str, mode: str):
        ...
        if self._var.get() == "":
            ...
            if not self._lb_open:
                ...
                self._place(x=self.winfo_x() + self.winfo_width(), y=self.winfo_y() + self.winfo_height() + 10)

                self._lb_open = True

            symbols_matched = [symbol for symbol in self._symbols.startswith(self._var.get())]

            if len(symbols_matched) > 0:

                try: 
                    self._ld.delete(0, tk.END)
                except tk.TclError:
                    pass 
