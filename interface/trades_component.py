...
class TradesWatch(tk.Frame):
    ...
    def add_trade(self, data: typing.Dict):
        ...
        self.body_widgets['status'][t_index] = tk.label(self._table_frame, text=data['status'], bg=BG_COLOR, fg=FG_COLOR_2, font=GLOBAL_FONT)

        self.body_widgets['status'][t_index].grid(row=b_index, column=6)

        self.body_widgets['pnl_var'][t_index] = tk.StringVar()
        self.body_widgets['pnl'][t_index] = tk.label(self._table_frame, text=data['status'], bg=BG_COLOR, fg=FG_COLOR_2, font=GLOBAL_FONT)
